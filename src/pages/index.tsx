...
const Home = () => {
  ...
  useEffect(() => {
    ...
    axios.get<Song[]>('https://jsonplaceholder.typicode.com/photos?_limit=10')
      .then(response => {
      ...
      <div className="w-full h-64 overflow-hidden rounded-lg shadow-lg mb-4">
        <Carousel showThumbs={false} infiniteLoop useKeyboardArrows autoPlay>
          {data.slice(0, 3).map((img, index) => (
            <div key={index}>
              <Link href={`/playlist/[id]?playlistData=${encodeURIComponent(JSON.stringify(img))}`} as={`/playlist/${img.id}`}>
                <motion.img
                  src={img.url}
                  alt={img.title}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
        ...
        <div className="col-span-2 bg-gray-700 p-6 rounded-lg shadow-lg space-y-4">
          ...
          {data.slice(0, 3).map((playlist, index) => (
            <div
              key={index}
              className="flex items-center space-x-4 p-4 rounded-lg shadow-md hover:bg-gray-900"
            >
              <Link
                href={`/playlist/[id]?playlistData=${encodeURIComponent(
              ...
              <div>
                <h3 className="font-semibold text-lg text-white">{playlist.title}</h3>
        ...
        {data.slice(3).map((item, index) => (
          <div key={index} className="bg-gray-700 p-6 rounded-lg shadow-lg">
            <h2 className="font-bold text-2xl mb-2 text-white">
              <Link href={`/playlist/[id]?playlistData=${encodeURIComponent(JSON.stringify(item))}`} as={`/playlist/${item.id}`}>
                {item.title}
              </Link>
            </h2>
